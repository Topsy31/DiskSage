...
import type { TreeNode, RecommendationItem, WebResearchResult } from '../types'
...
interface ExploreTabProps {
  ...
  onWebResearch: (item: RecommendationItem) => Promise<any>
  onMarkForDeletion: (path: string) => void
...
export default function ExploreTab({
  ...
  // Create a RecommendationItem for the selected node
  const selectedItem = useMemo(() => {
    if (!selectedNode) return null

    const rec = recommendations.find(
      r => r.entry.path.toLowerCase() === selectedNode.path.toLowerCase()
    )
    ...
    return {
      entry: {
        ...
        allocated: selectedNode.size,
        modified: new Date(),
        attributes: ''
      },
      classification: selectedNode.classification || {
      ...
      potentialSavings: selectedNode.size
    }
  }, [selectedNode, recommendations])

  const isMarked = selectedItem ? markedPaths.has(selectedItem.entry.path.toLowerCase()) : false
...
// Extended DetailPanel with mark for deletion
interface DetailPanelWithMarkProps {
